...
# read mG genus abundance and epibacteria list
epi=read.delim('epibiotic_bacteria_list.txt',header = F)
bc=read.delim('epibiotic_cyanobacteria_abundance.tsv',sep = '\t',header = T, row.names = 1)
rownames(bc) = gsub("_([679])","_0\\1",rownames(bc))
rownames(metadata) == rownames(bc)
##### correlation b/w cyano and epibiont #####
# the correlation between mG genus and PC for Microcystis
bc_sub = bc[colnames(bc) %in% c('cyano_Microcystis', 'cyano_Anabaena', 'cyano_Dolichospermum')]
bc$cyano_Microcystis
cor(metadata$PC, bc$cyano_Microcystis)
plot(metadata$PC, bc$cyano_Microcystis)
# pool all cyano genus abundance
cyano_cols <- grep("^cyano_", names(bc), value = TRUE)
# Add values across those columns (row-wise)
cyano_cols_val <- rowSums(bc[, cyano_cols], na.rm = TRUE)
cor(metadata$PC, cyano_cols_val)
plot(metadata$PC, cyano_cols_val)

# pairwise correlation between cyano with epibiotic bacteria and T
bc$Tempearture = metadata$T
cor_mat <- cor(bc, method = "pearson", use = "complete.obs")
# Set coefficients < 0.5 to zero
cor_mat_filtered <- cor_mat
cor_mat_filtered[abs(cor_mat_filtered) < 0.5] <- 0
# Choose Nature-style palette (e.g., blue-white-red)
col_palette <- colorRampPalette(c("#2166AC", "white", "#B2182B"))(100)

# Plot
corrplot(cor_mat_filtered, method = "color", col = col_palette, type = "upper",
         order = "hclust", tl.srt = 90, tl.cex = 0.5, tl.col = 'black')

###### identify the high correlted bacteria in general
mg = read.delim('mG_genus_abundance.txt', sep = '\t', header = T, row.names = 1)
cor_mat = cor(mg, method = "pearson", use = "complete.obs")
...
# Target taxa (columns to extract)
target_cols <- c("Microcystis", "Anabaena", "Dolichospermum")

# Your list of taxa to highlight in red
highlight_taxa = epi$V1

# Step 1: Filter rows with |r| > 0.7 in any target column
high_corr_rows <- apply(cor_mat[, target_cols], 1, function(x) any(abs(x) > 0.7))
high_corr_rows[rownames(cor_mat) %in% target_cols] <- FALSE  # Exclude self-correlations

# Step 2: Subset the matrix
filtered_mat <- cor_mat[high_corr_rows, target_cols]

# Step 3: Apply value transformation rules
filtered_mat <- apply(filtered_mat, c(1, 2), function(x) {
  if (x > 0 & x < 0.7) return(0.1)
  else if (x < 0 & x > -0.5) return(-0.1)
  else return(x)
})

# Step 4: Cluster rows and columns for better visualization
row_order <- hclust(dist(filtered_mat))$order
col_order <- hclust(dist(t(filtered_mat)))$order
ordered_rows <- rownames(filtered_mat)[row_order]
ordered_cols <- colnames(filtered_mat)[col_order]

# Step 5: Melt to long format
df_long <- melt(filtered_mat)
colnames(df_long) <- c("Taxa", "Reference", "Correlation")
df_long$Taxa <- factor(df_long$Taxa, levels = ordered_rows)
df_long$Reference <- factor(df_long$Reference, levels = ordered_cols)

# Step 6: Create color map for y-axis labels
taxa_colors <- ifelse(levels(df_long$Taxa) %in% highlight_taxa, "red", "black")
names(taxa_colors) <- levels(df_long$Taxa)

# Step 7: Plot heatmap using ggplot2
"#2166AC", "white", "#B2182B"
ggplot(df_long, aes(x = Reference, y = Taxa, fill = Correlation)) +
  geom_tile(color = "white") +
  scale_fill_gradient2(low = "#2166AC", high = "#B2182B", mid = "white", 
                       midpoint = 0, limits = c(-1, 1), name = "Correlation") +
  scale_y_discrete(
    labels = function(x) {
      # Render axis labels in red if in highlight list
      mapply(function(label, color) {
        sprintf("<span style='color:%s'>%s</span>", color, label)
      }, x, taxa_colors[x])
    }
  ) +
  labs(title = "Filtered and Transformed Correlation Heatmap",
       x = "Reference Taxa", y = "Highly Correlated Taxa") +
  theme_minimal(base_size = 12) +
  theme(
    axis.text.x = element_text(angle = 0, hjust = 1),
    axis.text.y = ggtext::element_markdown(size = 4),
    panel.grid = element_blank()
  )


###### use feature selecton spls for correlaiton b/w cyano and epibionts
# load library
