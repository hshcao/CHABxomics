...
#################################################################################
########---------- 1. DESeq2 and topGO and 2. WGCNA and topGO ----------########
######## 1. DESeq2 and topGO
## MA taihu98 ##
library(pheatmap)
...
rm(list=ls())
df=read.csv("Mar_mT19_allSamples_geneAbundance_readCount.txt", sep = "\t",header = T, row.names = 1)
df=df[rowSums(df)>0,]
mdf=matrix(0,nrow=nrow(df),ncol=ncol(df)/2)
rownames(mdf)=rownames(df)
colnames(df)=gsub("\\.","_",colnames(df))
colnames(mdf)=unique(gsub("_(1|2)$","",colnames(df), perl = TRUE))
for (d in 1:ncol(mdf)) {
  tempi=df[,grepl(colnames(mdf)[d], colnames(df))]
  mdf[,d]=rowMeans(tempi)
}
parts <- do.call(rbind, lapply(strsplit(rownames(mdf), "_"), function(x) {
...
rownames(mdf)=parts$locusID
colnames(mdf)=gsub("mT19_","mT", colnames(mdf))
colnames(mdf)=gsub("mT([679])_","mT0\\1", colnames(mdf))
mdf=mdf[,sort(colnames(mdf))]

down_list=up_list=list()
list_names=c('mt7v6', 'mt9v6', 'mt11v6', 'mt12v6')

n=4
cts=round(as.matrix(mdf))
#mt7v6
#cts=cts[, 1:6]
#mt9v6
#cts=cts[, c(1:3,7:9)]
...
#write.csv(as.data.frame(res05_order), 
#          file="condition_treated_results.csv")
resSig <- subset(resOrdered, padj < 0.05)

up_list[[list_names[n]]]=rownames(resSig)[resSig$log2FoldChange>0 & resSig$padj<0.05]
down_list[[list_names[n]]]=rownames(resSig)[resSig$log2FoldChange<0 & resSig$padj<0.05]
...
for (i in 1:length(up_list)){
  #i=1
  genesDN=down_list[[list_names[i]]]
  genesUP=up_list[[list_names[i]]]
  for (j in 1:2){
    if (j==1) {
      gene_list=genesUP
...
# Save workbook
saveWorkbook(wb, "taihu98_DEGs_topGO_4pairs.xlsx", overwrite = TRUE)
...
par(cex = 0.6);
par(mar = c(0,5,2,0))
pdf("mT19_taihu98_wgcna_all-samples_cluster.pdf", width = 5, height = 4);
pt<-plot(sampleTree, main = "mT19_taihu98_tpm", sub="", xlab="Sample", 
...
pdf("mT19_taihu98_wgcna_modules_metadata_corr_heatmap_4x5.pdf",4,5)
library(colorspace)
...
labeledHeatmap(Matrix = moduleTraitCor,
               ...
               zlim = c(-1,1),
               xLabelsAngle = 90,
               main = paste("mT19_Maeruginosa_metadata correlation"));
pheatmap(
  mat = moduleTraitCor,
...
#pdf("wgcanMT_taihu98_5module-T_negativeCorr_heatmap.pdf",8,8)
#picked_module = c("turquoise", "red","lightgreen","pink","grey60")
#outfile="wgcanMT_taihu98_1module-T_positiveCorr_topGO.tsv"
pdf("wgcanMT_taihu98_3module-T_positiveCorr_heatmap.pdf",8,8)
outfile="wgcanMT_taihu98_3module-T_positiveCorr_topGO.tsv"
picked_module =c("salmon","royalblue","darkgreen")

# # modules associated with PC
# pdf("wgcanMT_taihu98_1module-PC_negativeCorr_heatmap.pdf",8,8)
# outfile="wgcanMT_taihu98_1module-PC_negativeCorr_topGO.tsv"
# picked_module = c("turquoise")

# # modules associated with NH4-N
# pdf("wgcanMT_taihu98_3module-NH4-N_positiveCorr_heatmap.pdf",8,8)
# outfile="wgcanMT_taihu98_3module-NH4-N_positiveCorr_topGO.tsv"
# picked_module = c("turquoise", "magenta", "lightcyan")

# # modules associated with NO3-N
# pdf("wgcanMT_taihu98_3module-NO3-N_positiveCorr_heatmap.pdf",8,8)
# outfile="wgcanMT_taihu98_3module-NO3-N_positiveCorr_topGO.tsv"
# picked_module = c("lightgreen", "steelblue", "magenta")

# modules associated with NO2-N
# pdf("wgcanMT_taihu98_3module-NO2-N_positiveCorr_heatmap.pdf",8,8)
# outfile="wgcanMT_taihu98_3module-NO2-N_positiveCorr_topGO.tsv"
# picked_module = c("royalblue", "saddlebrown", "white")

# modules associated with Pi
# pdf("wgcanMT_taihu98_3module-Pi_positiveCorr_heatmap.pdf",8,8)
# outfile="wgcanMT_taihu98_3module-PI_positiveCorr_topGO.tsv"
# picked_module = c("brown", "yellow", "cyan")
...
for (i in 1:length(picked_module)){
  ...
  pheatmap(log10(t(module_df+1)), cluster_cols=FALSE, fontsize = 7,
...
# Create a named vector for mapping: names are old, values are new
...
#pheatmap(t(log10(combined_module_df + 1)), main = outfile,
#         cluster_cols=FALSE, fontsize = 6, show_rownames = TRUE)
dev.off()
...
########---------- 1. DESeq2 and topGO and 2. WGCNA and topGO ----------########
######## 1. DESeq2 and topGO
## Dolichospermum circinale AWQC310F: Acir310F ##
library(pheatmap)
library(RColorBrewer)
library(topGO)
library(DESeq2)
#### compare 12-4 with 11-3 ####
rm(list=ls())
df=read.csv("Acir_mT19_allSamples_geneAbundance_readCount.txt", sep = "\t",header = T, row.names = 1)
df=df[rowSums(df)>0,]
mdf=matrix(0,nrow=nrow(df),ncol=ncol(df)/2)
rownames(mdf)=rownames(df)
colnames(df)=gsub("\\.","_",colnames(df))
...
parts=as.data.frame(parts)
rownames(mdf)=parts$locusID
colnames(mdf)=gsub("Acir_mT19_","mT", colnames(mdf))
colnames(mdf)=gsub("mT([679])_","mT0\\1_", colnames(mdf))
mdf=mdf[,sort(colnames(mdf))]
df2=mdf[,13:18]
df2[,1]=rowMeans(df2[,2:3])
df2[,4]=rowMeans(df2[,5:6])
df2=df2[rowSums(df2>0) >3,]
dim(df2)

down_list=up_list=list()
list_names=c('mt12v11', 'mt9v6', 'mt11v6', 'mt12v6')
n=1
cts=round(as.matrix(df2))

coldata=data.frame(sample=colnames(cts),condition=gsub("_3_(1|2|3)|_4_(1|2|3)","",colnames(cts), perl = TRUE))
rownames(coldata)=coldata[,1]
coldata$condition <- factor(coldata$condition)
...
# Save workbook
saveWorkbook(wb, "acir310F_DEGs_topGO_4pairs.xlsx", overwrite = TRUE)
...
#mt7v6
#cts=cts[, 1:6]
#coldata=data.frame(sample=colnames(cts),condition=gsub("_15_(1|2|3)|_23_(1|2|3)","",colnames(cts), perl = TRUE))
#mt9v6
#cts=cts[, c(1:3,7:9)]
#coldata=data.frame(sample=colnames(cts),condition=gsub("_15_(1|2|3)|_10_(1|2|3)","",colnames(cts), perl = TRUE))
#mt11v6
#cts=cts[,c(1:3, 13:15)]
#coldata=data.frame(sample=colnames(cts),condition=gsub("_15_(1|2|3)|_3_(1|2|3)","",colnames(cts), perl = TRUE))
#mt12v6
cts=cts[,c(1:3, 16:18)]
coldata=data.frame(sample=colnames(cts),condition=gsub("_15_(1|2|3)|_4_(1|2|3)","",colnames(cts), perl = TRUE))
...
# Save workbook
saveWorkbook(wb, "acir310F_DEGs_topGO_4pairs2.xlsx", overwrite = TRUE)
...
geneAbd_mt=read.delim(genefiles[g],header = T,sep = "\t", row.names = 1)
colnames(geneAbd_mt)=gsub("\\.","-",colnames(geneAbd_mt))
colnames(geneAbd_mt)=gsub("Acir-mT19_", "mT", colnames(geneAbd_mt))
colnames(geneAbd_mt)=gsub("mT([679])-","mT0\\1-",colnames(geneAbd_mt))
dates=sort(unique(gsub("-[12]$","", colnames(geneAbd_mt))))
...
otu=as.data.frame(matrix(0,nrow=length(dates),ncol=nrow(geneAbd_mt)))
rownames(otu)=dates
colnames(otu)=rownames(geneAbd_mt)
for (d in 1:length(dates)) {
  #d=1
  tempi=as.matrix(geneAbd_mt[,grepl(rownames(otu)[d],colnames(geneAbd_mt))])
  tempii=apply(tempi,1,mean)
  otu[d,]=as.numeric(tempii)
}
otu=otu[,colSums(otu>0)>7]
logotu=log10(t(as.matrix(otu))+1)
all(rownames(otu)==rownames(datTraits))

library(pheatmap)
pdf("mT19_acir310F_log10tpm_heatmap.pdf",6,6)
pheatmap(logotu, cluster_cols=FALSE, show_rownames=FALSE, main = "log10TPM_mT19_MAR")
barplot(datTraits$T, names.arg = rownames(datTraits),
        las = 2, col = "lightblue", ylab = "Temperature (°C)", cex.names = 0.8)
dev.off()
...
par(cex = 0.6);
par(mar = c(0,5,2,0))
pdf("mT19_acir310F_wgcna_all-samples_cluster.pdf", width = 5, height = 4);
pt<-plot(sampleTree, main = "mT19_acir310F_tpm", sub="", xlab="Sample", 
         cex.lab = 0.8, cex.axis = 0.8, cex.main = 0.8, cex=0.8, ylim=c(0,0.5)
)
dev.off()
...
traitColors = numbers2colors(datTraits, signed = FALSE);
# Plot the sample dendrogram and the colors underneath.
pdf("Acir310F_wgcna_tpm_meta_combined.pdf",5,5)
plotDendroAndColors(sampleTree2, traitColors,
                    groupLabels = names(datTraits),
                    main = "Sample dendrogram and trait heatmap",
                    cex.colorLabels = 0.85, cex.dendroLabels = 1, 
...
moduleTraitPvalue = corPvalueStudent(moduleTraitCor, nSamples);
#rownames(moduleTraitCor)=gsub("ME",paste(sites[4+f], "_",sep = ""), rownames(moduleTraitCor))
listname= "Acir310F" #paste(sites[4+f],species[g],sep = "_")
...
dim(textMatrix) = dim(moduleTraitCor)
# Display the correlation values within a heatmap plot
pdf("mT19_Acir310F_wgcna_modules_trait_corr_heatmap_4x4.pdf",8,8)
library(colorspace)
library(pheatmap)
...
labeledHeatmap(Matrix = moduleTraitCor,
               ...
               textMatrix = textMatrix,
               setStdMargins = FALSE,
               cex.text = 0.3, cex.lab.x = 0.5, cex.lab.y = 0.5,
               zlim = c(-1,1),
               xLabelsAngle = 90,
               main = paste("mT19_Acir310F_metadata correlation"));
...
# modules associated with temperature
# pdf("wgcanMT_acir310F_2module-T_negativeCorr_heatmap.pdf",8,8)
# #pdf("wgcanMT_acir310F_1module-T_positiveCorr_heatmap.pdf",8,8)
# picked_module = c("cyan", "yellow")
# outfile="wgcanMT_acir310F_2module-T_negativeCorr_topGO.tsv"
# #picked_module =c("salmon")

# # modules associated with PC
# pdf("wgcanMT_acir310F_1module-PC_postiveCorr_heatmap.pdf",8,8)
# outfile="wgcanMT_acir310F_1module-PC_postiveCorr_topGO.tsv"
# picked_module = c("grey")

# # modules associated with NH4-N
# pdf("wgcanMT_acir310F_1module-NH4-N_positiveCorr_heatmap.pdf",8,8)
# outfile="wgcanMT_acir310F_1module-NH4-N_positiveCorr_topGO.tsv"
# picked_module = c("yellow")

# # modules associated with NO3-N
# pdf("wgcanMT_acir310F_3module-NO3-N_positiveCorr_heatmap.pdf",8,8)
# outfile="wgcanMT_acir310F_3module-NO3-N_positiveCorr_topGO.tsv"
# picked_module = c("lightgreen", "steelblue", "magenta")

#modules associated with NO2-N
# pdf("wgcanMT_acir310F_1module-NO2-N_positiveCorr_heatmap.pdf",8,8)
# outfile="wgcanMT_acir310F_1module-NO2-N_positiveCorr_topGO.tsv"
# picked_module = c("purple")

# modules associated with Pi
pdf("wgcanMT_acir310F_1module-Pi_positiveCorr_heatmap.pdf",8,8)
outfile="wgcanMT_acir310F_1module-Pi_positiveCorr_topGO.tsv"
picked_module = c("salmon")
...
for (i in 1:length(picked_module)){
  ...
  pheatmap(log10(t(module_df+1)), cluster_cols=FALSE, fontsize = 3,
...
#######################
######################
# plot the GO terms of Dolichosperumum circinale Acir310F
# Load libraries
# Load libraries
rm(list = ls())
# Load libraries
...
file_path <- "acir310F_DEGs_topGO_4pairs.xlsx"

# Function to plot a sheet
plot_GO_bubble <- function(sheet_name, text_size = 12) {
  
  # Read data
  df <- read_excel(file_path, sheet = sheet_name)
  
  # Ensure classicFisher is numeric
  df <- df %>%
    mutate(classicFisher = as.numeric(classicFisher))
  
  # Filter rows where classicFisher < 0.06
  df_filtered <- df %>%
    filter(!is.na(classicFisher), classicFisher < 0.06) %>%
    mutate(logFisher = -log10(classicFisher),
           # Reorder GO terms by logFisher for neat plotting
           Full_Term = reorder(Full_Term, logFisher))
  
  # Bubble plot
  p <- ggplot(df_filtered, aes(x = logFisher, y = Full_Term, 
                               size = Significant, color = classicFisher)) +
    geom_point(alpha = 0.7) +
    scale_color_gradient(low = "steelblue", high = "coral", 
                         name = "p-value") +
    scale_size_continuous(name = "Significant genes") +
    labs(x = expression(-log[10]~"(Fisher's p-value)"),
         y = "GO Term",
         title = paste("Bubble plot for", sheet_name)) +
    theme_bw() +
    theme(axis.text.y = element_text(size = text_size),
          axis.text.x = element_text(size = text_size),
          axis.title = element_text(size = text_size + 2),
          plot.title = element_text(size = text_size + 4, face = "bold"))
  
  return(p)
}

# Example usage
p1 <- plot_GO_bubble("up_mt12v11", text_size = 6)
p2 <- plot_GO_bubble("dn_mt12v11", text_size = 6)

pdf("Acir310F_DEG_GOterms_bubble_5x4.pdf",5,4)
# Print plots
print(p1)
print(p2)
dev.off()
